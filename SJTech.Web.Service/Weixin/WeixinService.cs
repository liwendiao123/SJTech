...
using SJTech.Core.Models.WeixinTemplate;
...
namespace SJTech.Service.Weixin
{
        ...
        public bool DownloadTemplate(string serverId, string fileName, bool getNewToken = false)
        {
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                if (topBuffer[0] == '{')
                {
                 ...
                 Log.LogUtility.Weixin.InfoFormat("下载失败：{0}。serverId：{1}", str, serverId);
                    return false;
                }
                ms.Position = 0;
                //创建目录
               ...
               Log.LogUtility.Weixin.InfoFormat("下载成功：Path[{0}]", fileName);
            }
            return true;
        }
